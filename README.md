...
This is built into [`/grill-with-docs`](./skills/engineering/grill-with-docs/SKILL.md). It's a grilling session that helps you build a shared language with the AI and document hard-to-explain decisions in ADRs.
...
A **[`/tdd`](./skills/engineering/tdd/SKILL.md) skill** is included and can slot into any project. It encourages red-green-refactor and gives the agent plenty of guidance on what makes good and bad tests.
...
- **[setup-repo-skills](./skills/engineering/setup-repo-skills/SKILL.md)** — Scaffold the per-repo config (issue tracker, triage label vocabulary, domain doc layout, orchestration-label policy) that the engineering skills consume. Run once per repo before using `to-issues`, `to-prd`, `triage`, `tdd`, `diagnosing-bugs`, or `improve-codebase-architecture`.
...
- **[to-prd](./skills/engineering/to-prd/SKILL.md)** — Turn the current conversation into a PRD and publish it to the issue tracker. No interview — just synthesizes what you've already discussed, including rollout/orchestration notes when useful.
- **[do-work](./skills/engineering/do-work/SKILL.md)** — Execute a GitHub issue using `to-orc` stage labels, either by selecting the next unblocked issue or by honoring an explicit target such as `do-work 67`, then choosing the correct `dev` vs worktree flow and branch base.
- **[to-orc](./skills/engineering/to-orc/SKILL.md)** — Design dependency stages, parallel lanes, and chat routing for implementation issues so multiple agent chats can execute safely.
